...
int main(int argc, char** argv)
{
    ...
    while (Game1.spotsCovered < 42 && !lost)
    {
        ...
        {display(Game1); cout << endl << "PLAYER " << currentTurn << " WON!!!" << endl;}
    }
...
bool winMaybe(BOARD g)
{
    ...
    {
        int cols = 0;
        int rCopy = rows; // can't change rows directly when iterating
        while (cols >= 0 && cols <= 6 && rCopy <= 5 && rCopy >= 0) // inefficient way to iterature, but someone can make it more efficient
        {
            
            if (g.game[rCopy][cols] == 0) // 0 resets counter
            {count = 0; player = 0;} 
            else if (g.game[rCopy][cols] != 0 && player == 0) // non-zero first move sets players
            {count++; player = g.game[rCopy][cols];}
            else if (g.game[rCopy][cols] == player) // increment
            {count++;}
            else
            {
                count = 0; player = g.game[rCopy][cols];
            }
            if (count ==4) // keep checking wins
            {return true;}
            rCopy--; cols++; // we going that way
        }
    ...
    {
        int rows = 5;
        int cCopy = cols; // can't change rows directly when iterating
        while (cCopy >= 0 && cCopy <= 6 && rows <= 5 && rows >= 0) // inefficient way to iterature, but someone can make it more efficient
        {
            
            if (g.game[rows][cCopy] == 0) // 0 resets counter
            {count = 0; player = 0;} 
            else if (g.game[rows][cCopy] != 0 && player == 0) // non-zero first move sets players
            {count++; player = g.game[rows][cCopy];}
            else if (g.game[rows][cCopy] == player) // increment
            {count++;}
            else
            {
                count = 0; player = g.game[rows][cCopy];
            }
            if (count ==4) // keep checking wins
            {return true;}
            rows--; cCopy++; // we going that way
        }
        
        count = 0; player = 0; // new column
    }
    // Diagonal Down Right
    for (int rows = 5; rows >=5; rows--)
    {
        int cols = 0;
        int rCopy = rows; // can't change rows directly when iterating
        while (cols >= 0 && cols <= 6 && rCopy <= 5 && rCopy >= 0) // inefficient way to iterature, but someone can make it more efficient
    ...
    return false; // if here, nothing happened
}